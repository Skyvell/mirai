...
- `infra/` — GCP infrastructure (OpenTofu). Cloud SQL + Cloud Run + a user-uploads GCS bucket + Vertex AI access, for MVP.
...
src-layout single package `mirai_api` (`src/mirai_api/`): `main.py` (app + CORS), `core/` (`config` settings, `db` Cloud SQL engine, `security` Clerk JWT verify, `deps` FastAPI dependencies), `models/` (SQLAlchemy declarative models — `Base`, `User`, `Biomarker`, `LabUpload`, `BiomarkerMeasurement`), `services/` (`storage` GCS client, `lab_parsing` Pydantic AI agent + pure `map_extraction`), `routers/` (`health` → `/healthz`, `/readyz`; `me` → `/me`; `lab_uploads` → `POST /lab-uploads`). Add further domain models/routers per MVP scope.

**Lab PDF upload → biomarkers.** `POST /lab-uploads` (multipart, Clerk-authed) stores the PDF in GCS (`users/{user_id}/labs/{upload_id}.pdf`), then Claude (`claude-opus-4-8` on Vertex AI, via Pydantic AI) parses it into `biomarker_measurements` mapped against the seeded `biomarkers` catalogue (`slug` is the LLM key; `loinc_code`+UCUM `canonical_unit` for future FHIR). Parsing is synchronous in-request; blocking GCS/DB work runs via `run_in_threadpool` (no async SQLAlchemy). Extra settings: `GCS_BUCKET`, `GCP_PROJECT_ID`, `VERTEX_REGION` (all keyless via ADC). Units stored verbatim; canonical conversion is `[LATER]`.
