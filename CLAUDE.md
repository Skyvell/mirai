...
src-layout single package `mirai_api` (`src/mirai_api/`), layered with domain-named files: `main.py` (app assembly + lifespan: logging config, engine warm-up), `core/` (`config` settings, `db` Cloud SQL engine, `security` Clerk JWT verify, `deps` FastAPI dependencies, `enums` shared vocabulary like `UploadStatus`), `models/` (SQLAlchemy, one file per model; `base.py` holds `Base` with the Alembic naming convention; `__init__.py` aggregates), `schemas/` (Pydantic API models, one module per router), `routers/` (`health`, `me`, `biomarkers`, `lab_uploads`), `services/` (`storage` GCS client, `lab_parsing` Pydantic AI agent + pure `map_extraction`, `lab_uploads` persistence). New domain feature = model + schema + router (+ service when logic warrants). Revisit module-per-domain packages when a second real domain (physiology) lands.
