...
**Lab PDF upload → async parse → review → commit.** Extraction is error-prone, so LLM output never enters the record unreviewed; it lands as a draft the user confirms. `LabUploadService` (`lab_upload_repository` + `draft_biomarker_measurement_repository` + `biomarker_repository` + `Session`) owns the transaction, mirroring the biomarker layering. Lifecycle (`core/enums.UploadStatus`): `pending → processing → awaiting_review → committed`, with `failed` terminal from `processing`.
- `POST /lab-uploads` (multipart, Clerk-authed, **202**) stores the PDF in GCS (`users/{user_id}/labs/{upload_id}.pdf`) as `pending`, then dispatches parsing: to Cloud Tasks when `tasks_enabled`, else in-request (local/dev). The worker `POST /internal/lab-uploads/{id}/parse` (OIDC-gated, not user-facing) claims the row via CAS (`pending→processing`, idempotent on redelivery), pulls the PDF from GCS, runs Claude (`claude-opus-4-8` via Pydantic AI), and writes `draft_biomarker_measurements` (mapped + unmatched markers), reaching `awaiting_review`. A parse failure is terminal (`failed`); an infrastructure error resets to `pending` and re-raises so the queue retries. Stuck `pending`/`processing` rows older than ~10 min read as `failed` (no reaper).
- `GET /lab-uploads/{id}` returns status + the nested draft while awaiting review; `PATCH /lab-uploads/{id}/draft` edits it (keep/drop rows, correct values, map an unmatched marker to a catalogue `slug`); `POST /lab-uploads/{id}/confirm` commits the kept, mapped rows into `biomarker_measurements` (via `BiomarkerRepository.add_measurements`) and flips to `committed` (idempotent; draft rows retained as audit). The biomarker series API reads only committed measurements, so drafts never leak in.
...
- Settings: `GCS_BUCKET`, `GCP_PROJECT_ID` (keyless via ADC), `ANTHROPIC_API_KEY` (Secret Manager on Cloud Run; `backend/.env` locally), `UPLOAD_ALLOWLIST` (comma-separated user UUIDs — LLM cost gate; empty allows all), and the async-parse group `TASKS_ENABLED`/`TASKS_QUEUE`/`TASKS_LOCATION`/`TASK_INVOKER_SA`/`WORKER_BASE_URL` (all tofu-set; empty `WORKER_BASE_URL` keeps parsing synchronous).
...
**API client — generated (`@hey-api/openapi-ts` + TanStack Query).** `pnpm generate:api` dumps the backend's OpenAPI schema (`backend/scripts/export_openapi.py` — importing the app has no side effects, no env needed) and regenerates `src/client/` (fetch client, typed SDK, `*Options()`/`*Mutation()` TanStack Query helpers keyed on the backend's `operation_id`s). Run it whenever the backend contract changes; `src/client/` is committed (CI's frontend job is pnpm-only), the intermediate `openapi.json` is gitignored. The client configures itself at creation via hey-api's `runtimeConfigPath` → `createClientConfig` in `src/lib/api.ts` (`baseUrl` from `VITE_API_URL`, Clerk token via hey-api's `auth` — attached only to operations the spec marks Bearer-secured, `HTTPBearer` in `core/deps.py`); `ApiProvider` in `main.tsx` (inside `ClerkProvider`) just injects Clerk's render-scoped `getToken`. `QueryClientProvider` wraps the router; queries refetch only on explicit invalidation (`staleTime` 5 min, no focus refetch), except the Sources report list which polls (`refetchInterval`) while any upload is `pending`/`processing`. Routes consume `currentUserOptions`, `listBiomarkersOptions`, `uploadLabMutation` (fire-and-forget: 202 → invalidates `listLabUploadsQueryKey`; parsing is async), the Sources list (`listLabUploadsOptions` + `deleteLabUploadMutation`), and the draft review route `/sources/$uploadId/review` (`getLabUploadOptions` + `updateLabDraftMutation` + `confirmLabUploadMutation`, confirm invalidates the series). `apiErrorMessage` surfaces FastAPI's `detail` from thrown error bodies.
