...
src-layout single package `mirai_api` (`src/mirai_api/`): `main.py` (app + CORS), `core/` (`config` settings, `db` Cloud SQL engine, `security` Clerk JWT verify, `deps` FastAPI dependencies), `models/` (SQLAlchemy declarative models — `Base` + `User`), `routers/` (`health` → `/healthz`, `/readyz`; `me` → `/me`). Add further domain models/routers per MVP scope.
