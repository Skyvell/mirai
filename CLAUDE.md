...
**Lab PDF upload → biomarkers.** `POST /lab-uploads` (multipart, Clerk-authed) stores the PDF in GCS (`users/{user_id}/labs/{upload_id}.pdf`), then Claude (`claude-opus-4-8` via the Anthropic API, through Pydantic AI) parses it into `biomarker_measurements` mapped against the seeded `biomarkers` catalogue (`slug` is the LLM key; `loinc_code`+UCUM `canonical_unit` for future FHIR). Parsing is synchronous in-request; blocking GCS/DB work runs via `run_in_threadpool` (no async SQLAlchemy). Extra settings: `GCS_BUCKET`, `GCP_PROJECT_ID` (keyless via ADC) and `ANTHROPIC_API_KEY` (Secret Manager on Cloud Run; `backend/.env` locally). Units stored verbatim; canonical conversion is `[LATER]`.
