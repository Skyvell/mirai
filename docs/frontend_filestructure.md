...
- **`features/overview/` exists ahead of its content**, unlike those four, because the dashboard it will hold composes biomarkers, wearables and omics — it is a settled fan-in slice, so churning the directory out and back is worse than the inconsistency.
