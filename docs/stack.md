...
### Migrations — [MVP]
Alembic (configured via `[tool.alembic]` in pyproject). SQLAlchemy models are the schema source of truth; migrations are autogenerated and reviewed. Applied by a dedicated Cloud Run job that CI executes before each service deploy — the API never runs DDL.
